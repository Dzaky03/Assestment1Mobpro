...
class GameFragment : Fragment(), View.OnClickListener {
    ...
    private lateinit var nameOfPlayer: GameFragmentArgs
    ...
    override fun onCreateView(
        ...
        nameOfPlayer = GameFragmentArgs.fromBundle(requireArguments())
        ...
        when(v?.id) {
            ...
            R.id.btnSubmit -> {
                if (mSelectedPosition == 0){
                    mCurrrentPosition++
                    when {
                        mCurrrentPosition <= mQuestionsList.size -> setQuestion()
                        else -> {
    ...
    private fun answerView(mSelectedPosition: Int, drawableView: Int) {
        when (mSelectedPosition) {
            ...
            4 -> binding.tvOptionFour.background = context?.let { ContextCompat.getDrawable(it, drawableView) }
        }
    }
}
